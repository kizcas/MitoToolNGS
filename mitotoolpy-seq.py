...
def manualModifyVariant(species, variantList):
    ...
    variantNeedModify = {"cattle": [["215+TCC"], ["16200", "16200+A"], ["12171-12173d"]],
                         ...
                         "horse": [["356d"], ["5210d"], ["16348-16355d"], ["16403-16404d"], ["355+C"], ["16158-16357d"],
                                   ["16334-16357d"]], "pig": [["136+C"], ["136+CA"]],
                         "sheep": [["16343", "16343+C"], ["567G", "568C"], ["15092G", "15093C", "15095A", "15096C"]],
                         "yak": [["880+GC"], ["744+TC"], ["893+GTGGGG"]]}
    variantAfterModify = {"cattle": [["216", "221+CCC"], ["16199+A", "16200"], ["12173-12175d"]],
                          "chicken": [["3941", "3946d"], ["3941", "3945-3946d"]],
                          "dog": [["15911", "15918d"], ["797", "799+C"], ["16019"]],
                          "goat": [["177-178d", "180T", "181A"]],
                          "horse": [["356", "358d"], ["5210", "5217d"], ["16127-16134d"], ["16403d", "16404d"],
                                    ["356", "356+T"], ["16153-16352d"], ["16329-16352d"]],
                          "pig": [["137C", "142+XA"], ["137C", "142+XA"]],
                          "sheep": [["16342+C", "16343"], ["566+G", "569d"], ["15091+G", "15097d"]],
                          "yak": [["881G", "892+CC"], ["745", "751+CC"], ["892+GGTGGG"]]}
...
def analyzeSeqInFile(species, inputFile, output, exeDir, region):
    ...
    regionSetDloop = {"cattle": [363, 15792], "chicken": [1, 1232], "dog": [15461, 16196], "horse": [15466, 16657],
                      "pig": [1, 1254], "yak": [1, 892], "sheep": [15437, 16616],
                      "goat": [15431, 16642]}  # the dloop of cattle is different from others
    regionSetWhole = {"cattle": [1, 16338], "chicken": [1, 16785], "dog": [1, 16196], "horse": [1, 16657],
                      "pig": [1, 16690], "yak": [1, 16322], "sheep": [1, 16616], "goat": [1, 16642]}
    regionSetNonDloop = {"cattle": [364, 15791], "chicken": [1233, 16785], "dog": [1, 15460], "horse": [1, 15465],
                         "pig": [1255, 16690], "yak": [893, 16322], "sheep": [1, 15436], "goat": [1, 15430]}
    regionBegin = 0
    regionEnd = 0
    treeFilePath = ""
    if region == "dloop":
    ...
    if region == "whole" or region == "dloop" or region == "nondloop":
        ...
        for line in treeFile:
            refHaplogrupName, refHaplogrupVariants = line.rstrip('\n').split(':')
            refVariantsSet = set(
                removeVariantForNonSelectedRegion(re.split(r',\s*', refHaplogrupVariants), regionBegin, regionEnd,
                                                  species, region))
            treeHaplogroupNameList.append(refHaplogrupName)
            if len(refVariantsSet) != 0:
                treeHaplogroupVariantsSetList.append(refVariantsSet)
    ...
    for query in queryRecord:
        ...
        querySetTmp = set(removeVariantForNonSelectedRegion(
            removeVariantForSomeRegion(species, manualModifyVariant(species, list(querySet))), regionBegin, regionEnd,
            species, region))
        querySet = {}
...
try:
    options, args = getopt.getopt(sys.argv[1:], "vhs:r:i:o:",
                                  ["version", "help", "species=", "region=", "input=", "output="])
...
if species not in ("cattle", "chicken", "dog", "goat", "horse", "pig", "sheep", "yak"):
    print(
        "Please set an available domestic animal using -s or --species, the candidates include cattle, chicken, dog, goat, horse, pig, sheep, and yak.")
    sys.exit()

print("============ Parameter ============")
if inputFile != "":
    print('python "' + os.path.abspath(__file__) + '"', " -s " + species, " -r " + '"' + region + '"',
          " -i " + '"' + inputFile + '"', " -o " + '"' + output + '"\n')
else:
    print('python "' + os.path.abspath(__file__) + '"', " -s " + species, " -r " + '"' + region + '"',
          " -i " + '"' + inputDir + '"', " -o " + '"' + output + '"\n')
